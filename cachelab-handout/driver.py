#!/usr//bin/python
#
# driver.py - The driver tests the correctness of the your cache
#     simulator and the correctness and performance of your transpose
#     functions. It uses ./test-csim to check the correctness of the
#     simulator and it runs ./test-trans on two different sized
#     matrices (32x32 and 64x64) to test the correctness and
...
def main():
    ...
    if miss32 == 2**31-1 :
        misses = "invalid"
    print "%-22s%8.1f%10d%12s" % ("Transpose performance 32x32", trans32_score, 
                                  maxscore['trans32'], misses)

    misses = str(miss64)
    if miss64 == 2**31-1 :
        misses = "invalid"
    print "%-22s%8.1f%10d%12s" % ("Transpose performance 64x64", trans64_score, 
